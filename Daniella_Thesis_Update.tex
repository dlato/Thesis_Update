...
\ch added some more references to the substitutions paper intro

\ch updated results from re-running substitutions analysis. (see tables below)
...
Looking into it a bit more, and it seems as though for the rest of the bacteria there is less data at a lot of the 10kb sections than there should be. 
So I need to figure out what is happening here.

\textbf{\dn/\ds $>$ 1 for some genes}

Excluding \strep, the rest of the bacteria have very few ``genes'' that have have \dn/\ds $>$ 1 (~4\% of all ``genes'').
Looking into this further these ``genes'' are actually segments of  genes that were really short (average of 33bp). This was because there were gaps in the genes so the rest of the gene got ``cut'' so it makes sense why these genes have such high $\omega$. I am not sure if we should be discarding these gene fragments from the analysis? Thoughts? 

\textbf{Weird \strep selection issue}

I already talked to you about this, but I looked into why \strep has so many genes with $\omega$ $>$ 1.
The alignments are all correct and the genomic positions are all correct. So \p and MAFFT are grabbing the correct genomic sequence.
I BLASTed a coding gene from one of the blocks and it does match with sequences from all the other species of \strep, however they are not found within the same block.
My speculation is that \p can not handle the slight differences between \sven and \scoe and \sliv and cuts the genome into too many blocks, classifying what genes are homologous.
This can also be seen in the \p block alignments where it looks like there is a clear divide between the sequence content of \scoe and \sliv and all the \sven strains.
There is also very few sections (just by looking at the alignment) where all 6 taxa have sequence data and no gaps.
Additionally, the branch lengths between the \scoe and \sliv strains are very small compared to the branch lengths between all the \sven strains.
See below for what I plan to do next to fix all this.

\textbf{Alignment split} 

I noticed that one of the blocks in \strep failed to be split into coding and non-coding sections.
I thought it was because I did not properly define an array v.s. a string for one of my variables, but I think something else is going on.
...
\begin{itemize}
	\item look for more \strep complete genomes to use in the analysis
	\item look into gblocks to assess MAFFT alignments
	...
	\item deal with genes where $\omega$ $>$ 1
\end{itemize}
