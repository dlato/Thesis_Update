...
\ch re-calculate $\omega$ excluding the values that should be undefined or zero

\ch first interpretation of \dn \ds results

\ch boxplots of \dn, \ds, and $\omega$ for each bacteria

\ch apply to GSA travel scholarship

\textbf{dN, dS, $\omega$}

I re-calculated the average $\omega$ values excluding anything that had \ds = 0 or both \dn and \ds = 0 which should give $\omega$ a value of 0 or being undefined. The results make WAY more sense now and are in Table \ref{tab:dN_dS_ratios}.

For the \dn, \ds results we see that for all the replicons that have $\omega$ < 1, \dn < \ds, as it should be.
...
\textbf{Boxplots}
I made boxplots for each bacterial replicon and they can be seen below.
For \ecol and \bas we can see there are a lot of outliers in both \dn and \ds.
For \pa and \pb there are a few outliers in \dn but none anywhere else.
\smel Chromosome looks really weird because there are so many 0 values for \dn, \ds, and $\omega$.
Thoughts on these?

\textbf{Inversions and Gene Expression:}
Still not sure if I should be using the \p or PARSNP alignments for this analysis. I think PARSNP is better because it gives even tiny inversions that \p sometimes misses. But \p has the core and pan-genome, so I am not sure if that is more ideal.
But we are dealing with the same strains of \ecol so most genes should be a part of the core and we do see that most of the genome is included in the alignment (see picture below). So maybe PARSNP is fine?
...
I also started to work on getting genomic positions associated with each \dn, \ds and $\omega$ value to show the distribution of these across the genome.
...
I want to get graphs showing the distribution of \dn, \ds, and $\omega$ across genomic position.
