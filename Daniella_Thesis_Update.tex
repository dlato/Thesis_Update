...
\ch updated the \strep selection figures to have negative positions (to match the substitution graph)
...
\ch add in reviewers comments from gene expression paper about why we chose these bacteria to substitution paper
...
Another thing I notices while looking into the selection analysis for \smel chromosome is that sometiems PAML puts a \ds value of 0.0001 even though there are no substitutions at all.
I am not sure how often this happens and if these values (0.0001) are only associated with no substitutions.
\textbf{Thoughts on what to do with these values?}
...
\begin{itemize}
	...
	\item make small changes based on Brian's comments from questions above
	\item update methods in paper draft (make sure they are the same as what I actually did bc things have changed a lot)
	\item create a script that will match up homologous genes from different \ecoli genomes for Queenie to use for the inversions project
\end{itemize}
...
\item  continue to update methods for substitutions paper
\end{itemize}
