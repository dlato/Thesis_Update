...
\ch done with mapping BW genes to K-12MG genes

\ch Queenie to create a toy dataframe that I can begin working on the analysis of comparing inverted and non-inverted sections of the genome

\ch Queenie: minimal genes found in overlapping regions (~0.5\%)
...
I finished mapping the BW genes to the K-12MG genes. So I now have a list of all the genes that are similar between those two strains. I have also repeated this for the other proteomes that were labelled as ``redundant'' so that I can ensure we are only looking at genes that actually exist in the strains we are looking at.
I first aligned the two genomes for mapping with PARSNP. I think split the alignment into it's blocks, re-aligned with mafft and ran this through my substitutions code (which is fairly conservative) to find sections of the alignment which are well aligned (no gaps, start at the same codon position, min 100bp length) and then printed out which gene was in each of these sections from each genome. \textbf{does this sound like an ok plan? It is essentially using the same pipeline we used in the substitutions paper.}

Queenie quantified that there is only 0.05\% of genes that are found in overlapping regions which I think is fine to just chuck and not use in the analysis.
There was not a lot of information about these genes (i.e. not peseudo genes) so I assume that the overlap is just PARSNP being unsure about the definitive borders of the blocks.

Queenie has set up a small ``toy'' dataframe with (some of) the gene expression and inversion info so I can start to play with this and develop some sort of analysis pipeline so that when the final dataframe is ready, we can run it through the analysis quickly!
...
%
\begin{itemize}
	\item think about how I am defining inversions and if I should classify inversions based on each strain vs if there is an inversion present in the block
	\item start playing with ``toy'' dataframe to determine analysis directions
...
\begin{itemize}
	...
	\item continue playing with ``toy'' dataframe to determine analysis directions
\end{itemize}
