...
\ch pick which \strep group of species to use

\ch make tree for new \strep species

\ch assess and re-align the \agro strains

\ch assess \borrb \p alignments

\ch finalise Gblocks pipeline, run \ecol through Gblocks pipeline to work out any kinks

\textbf{\strep}
I decided that using species for \strep that had more rearrangements was better than using more taxa.
So I will be using the one \scoe and two \sliv genomes for my analysis.
I started this by creating a new tree with branchelengths and bootstrap values for this taxa. 

\textbf{Other Linear Bacteria \p Alignments}
I assessed the \p alignments for \borrb and all of the strains are too similar and \p can only find one LCB. So I can not use this bacteria.
I also assessed the \p alignments for \agro and found that there were combinations of the strains that yielded between 32-66 LCBs.
This could be an option for my analysis. However, if I decide to use \agro then I think I will have to analize the circular and linear chromosomes, I can not just pick one.
So, we decided to put this portion of the project on hold for now and focus on fixing the \strep issues.
...
\textbf{Gblocks pipeline}
Last week I finalised incorporating Gblocks into my pipeline and ran it on \ecol just to work out any final kinks.
I began re-running the substitution analysis on all the bacterial strains last week and will continue to run that this week.
...
\begin{enumerate}
	...
	\item finish re-running subs analysis on all bacteria
	\item re-run dN/dS analysis on all bacteria
...
Gene Expression:
\begin{enumerate}
	\item non-linear model for gene expression to look at periodicity?
	\item new graphs for gene expression? (negative positions)
	\item new graphs for substitutions? (negative positions), write new (and better) code for this
