...
\begin{table}[h]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{lrr}
			\toprule
			& \multicolumn{2}{c}{dN/dS Ratio}\\
			\cmidrule{2-3}
			Bacteria and Replicon & Gene Average & Genome Average\\
			\midrule
			\ecol Chromosome &  & 0.0612  \\
			\bass Chromosome &   & 0.0911 \\
			\strep Chromosome &  &  \\
			\smel Chromosome &   & 157.0089 \\
			\smel pSymA &  & 30.1216 \\
			\smel pSymB &  & 21.0320\\
			\bottomrule
		\end{tabular}
	...
	\caption{\label{tab:dN_dS_ratios} dN/dS ratio calculated for each bacteria on a per gene and per genome basis. The per gene average was calculated by taking the average dN/dS of each gene, and then averaging these. The per genome average was calculated by taking the average of all dN/dS ratios.}
\end{table}
...
\begin{table}[h]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{lcc}
			...
			\smel pSymB & 2.878\e{-7}*** & 8.591\e{-7}***\\
			\bottomrule
		\end{tabular}
