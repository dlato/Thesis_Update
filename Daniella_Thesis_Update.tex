...
\ch double check github is up to date
...
\ch edited one section of dissertation intro
...
Queenie looked into how often blocks overlap but are not completely within another block (~5.4\%). From these blocks, ~26\% - ~ 37.3\% of genes are split between two blocks. I think that this is likely PARSNP not always getting the end/start position of a block completely correct. I am having Queenie check what kinds of genes are being split (paeudo genes..etc)
and if these genes are within an inversion or not.
...
I have a code that will retrieve the reciprocal best blast hits and print out the gbk gene name (to help with comparison to the parsnp output).
However, it appears as though the gene names from blast do not always match what is in the gbk file. This might be a version thing so I will keep looking into it.
...
\begin{itemize}
	...
	\item figure out how to assess synteny when the gene names do not match
...
\begin{itemize}
	...
	\item continue to work on blast synteny and BW/K-12 map
\end{itemize}
