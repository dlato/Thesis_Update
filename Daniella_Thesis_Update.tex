...
\ch edits to gene expression paper intro

\ch re-run selection analysis with fixed gene complement issue

\ch look into/test Gblocks alignment assessment

\ch look into other \strep genomes available

\ch figure out why \strep failed to split the alignment into coding and non-coding


\textbf{Gblocks}
So I looked into Gblocks and what it does. It seems very similar to trimal, so it breaks the alignment up into good and bad segments.
It spits out this big alignment into phylip interleaved format so it should be pretty easy to write a code to parse this and get the alignment into a format I can use.
So for \strep, Gblocks will throw out between 10\% - 99\% of the alignment for all the blocks.
It also had only 30\% of the blocks where 50\% or more of the sequence was kept.
The segments of alignment that it kept were between 10bp - 500bp.
Comparing this to \ecol, 67\% of the blocks had 50\% or more of the sequence kept and the segments of alignment that were kept were much longer than in \strep and ranged from 10bp - 30000bp.
Some of these chunks (for \strep and \ecol) are really small and I think these small segments are also showing up in my code and giving weird selection coefficients.
So I think having some sort of alignment length cut off, plus assessing the alignment with Gblocks will help eliminate these and then the selection results will hopefully look better and be more accurate.

I then looked closely at what Gblocks does v.s. what my code does.
Most of the time my code and Gblocks lines up, BUT there are cases where I kept something that Gblocks tossed, and cases where Gblocks kept something that I tossed.
So I am not sure what this means but I think using both will be the most stringent.

I also looked at the smaller segments of the alignment of \ecol and found that the alignment was not aligning homologous sequences either, but only for the smaller segments.
So I think we can't trust anything that is really short because we just can not be confident that it is correct.


\textbf{\dn/\ds $>$ 1 for some genes}

As mentioned above, these really short segments are just inaccurate. So I think making a cut off and using Gblocks should hopefully fix this issue

\textbf{\strep Alternate Genomes}

I found a paper that shows multiple phylogenetic trees for \strep and found some consistent groupings of strains.
I tried to find genomes related to \sven but when I did \p on these, there was still way too many blocks.
I am going to talk to Marie to see if she has any insight on this.
There may be more hope for genomes related to \scoe, so I am going to look into this more and see if I can get a \p alignment that is less crazy.

\textbf{Alignment split} 

The reason why one of the blocks for \strep failed to split is that the sequences are not homologous.
The code is a bit complicated to explain but the reason that it failed is that some taxa had an extra ``gene'' than others.
But again, this is because the sequences are not homologous so this should be fixed when I implement the above.
...
\begin{itemize}
	\item look for more \strep phylo trees and associated genomes (\scoe)
	\item check coding/non-coding code to see how often sites do not match up for \strep
	\item check how often the tree for each block matches the overall tree for \strep
	\item figure out what is going on with cod subs $>$ non-cod subs
\end{itemize}
