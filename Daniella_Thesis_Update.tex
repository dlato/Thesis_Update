...
\ch freed up some space in my home directory and moved some things to scratch on 115

\ch gather notes on inversions and gene expression papers

Edits to the Substitution Paper:

\ch Caitlin's edits to the paper

\ch Brian's edits to the discussion/conclusion

\ch Brian's edits to the supplemental \smel chromosome selection tests

\ch put all code for paper up on GitHub (private repo for now)

\ch added in nucleotide alignment of high substitutions example to better show lots of subs

\ch personal edits to discussion, re-ordered some paragraphs

\ch edit abstract for proper length

I made lots of little edits and wrote more for the substitution paper (see above checklist).
\textbf{The substitutions paper draft is almost complete so if you have any additional tests/analysis for this paper that you think I should do please tell me now so I can get to work on that. Otherwise, I think the analysis is done and the paper is ready for editing.}

I would like to get the formatting ready for submission of the paper (so it is a quick process when we are ready to submit). Last time we discussed this we talked about submitting to GBE,
\textbf{please let me know if this is still the plan or if you think another journal would be better.}
...
%
\begin{itemize}
	\item send Brian discussion to edit one more time
	\item check on Queenie's progress and double check her normalization code
	\item organize inversions and gene expression notes into broad categories
	\item create outline for intro of inversions and gene expression paper
	\item begin to look into how to perform blast to confirm inversions
\end{itemize}


\section*{Next Week}
\begin{itemize}
	\item Brian's discussion edits (round 2)
	\item send Brian whole paper to edit
	\item write good chunk of intro for inversions and gene expression paper
	\item blast portion of the analysis for gene expression and inversions paper
\end{itemize}
