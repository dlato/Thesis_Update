...
\ch finished latest draft of second round of revisions
...
\ch finished first draft of inversions paper (minus the DESeq analysis)

\ch looked at some HGT genes and inversions

\ch phylogentic tree for the taxa

\ch HNS and expression values for bound and unbound regions


\textbf{Dissertation:}

\ch Finished inversions proposed studies

\ch Finished inversions conclusion thesis summary

\ch Started inversions discussion
...
\textbf{I do not know what to do. I need to account for variation in expression between datasets (since they were all done in different labs at different times), while still being able to see what differences are due to the ``treatment'' $=$ inversions. Do you know how I can achieve this?}
 
I realized that I was considering a ``match'' between HNS and my inversions if any part of an HNS binding site overlapped with any region within the entire inverted block. This meant that each gene within an inverted block would be considered ``bound'' to HNS.
I think this is wrong, so I instead considered a ``match'' to be if any part of the HNS binding site overlapped with any portion of a gene. This way each individual gene would be assessed as to if it had HNS bound or not.
Figure \ref{fig:inver_hns} and Table \ref{tab:HNS} are updated with this information.
There is now complete consistency in sign between the significant correlations (Table \ref{tab:HNS}).
However, I am not sure which genomes to use for the HNS binding.
The data for HNS binding sites is based on the \ecol K-12 MG1655 strain. So I think that I should only be looking at this genome.
However, the \ecol K-12 MG1655 strain has no inverions, they mostly occur in the \ecol ATCC strain.
So I am considering an inverted alignment block as having at least one taxa inverted.
\textbf{Do you think it is ok that I am only looking at \ecol K-12 MG1655 and using the above criteria for inverted blocks to see if there is a correlation between HNS and inversions?}
 
Since a lot of the genes that HNS represses are linked to being recently horizontally transferred, I obtained the HGT information from the significant HNS datasets which is from previously published studies on HGT.
I looked at the correlation (Pearson) between HGT genes and inverted sequences, and HGT and blocks with significant differences in gene expression and I found no significant correlation for either.
\textbf{Should I be pulling HGT data from other sources to look into this? I just thought it would be good to at least touch on this in the paper since HGT genes are mentioned in a lot of the HNS literature.}
...
%
\begin{itemize}
	\item get HGT info for Oshima HNS data
	\item do HGT correlations on $\uparrow$ data
	\item Dissertation: discussion on inversions paper
	\item Dissertation: intro for inversions paper
	\item actual analysis on DESeq data
	\item visualizations/results for $\uparrow$
	\item start working on exit seminar presentation
...
\section*{Next Week}
\begin{itemize}
	\item continue working on exit seminar presentation
	\item go over missing spots/red marks in conclusion
	\item edit conclusion
	\item Brian's edits to inversions paper
...
\begin{table}[h]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{lccc}
			...
			\midrule
			Grainger 2006 \cite{Grai:06} & NS & NS & 53\\
			Ueda 2013 \cite{Ueda:13} & NS & NS & 275\\
			Higashi 2016 \cite{Higa:16} & &&\\
			\hspace*{10mm} criteria A & 0.0467* & NS & 371\\
			\hspace*{10mm} criteria B & 0.0540** & NS & 343\\
			\hspace*{10mm} criteria C & 0.0540** & NS & 343\\
			\hspace*{10mm} criteria D & 0.0540** & NS & 343\\
			\hspace*{10mm} criteria E & 0.0544** & NS & 340\\
			\hspace*{10mm} criteria F & 0.0544**  & NS & 340 \\
			Lang 2007 \cite{Lang:07} & 0.0574** & NS & 115\\ %composite data
			Oshima 2006 \cite{Oshi:06} & 0.0390* & NS & 664\\
			\bottomrule
		\end{tabular}
