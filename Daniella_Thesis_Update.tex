...
\ch Queenie: complete dataframe for inversions and gene exp!

\ch Queenie: working on getting summary statistics for inversions (how many, how big, in what taxa, where are they located, expression averages..etc)

\ch started to create mapping file to show which genes match in all the strains in each block (based on subst paper ``good'' alignments)

\ch started looking into genes that were missing from BW and K12MG mapping file

Subst Paper:

\ch added more recent references (chromids and plasmids)

\ch add program parameters to main text

\ch better explanation for removing \dn$=$\ds$=$0 and \dn$=$0

\ch better explanation for normalization for number of sites

\ch commented on multiple rep termini

\ch addressed origin scaling figure

\ch clearer figure captions

\ch started running larger window size supplemental tests

\ch \p for \smel chrom with 23 seqs (trying the analysis on this first)

\ch research how many more complete genomes there are
...
Queenie is working on creating summary statistics and graphics for the inversions (how big are they, in what taxa..etc.). \textbf{If you have any things you would like to know regarding this, please let me know!}

I have been trying to create a mapping of which genes match genes in all taxa (based on the alignment) so this can be used to compare genes in inverted and non inverted regions.
...
I have spent most of the week working on the simple revisions from the paper.
Adding extra references, explanations, and small analysis (larger window size).
...
%
\begin{itemize}
	\item complete mapping of all genes in all strains
	\item get Queenie to use $\uparrow$ information in her summary statistics of inversions
	\item get Queenie to start creating visualizations on the summary statistics for inversions
	\item complete larger window size supplementary analysis
	\item  keep working on scaling up the subst data
	\item add in new paper notes to subst intro
...
\section*{Next Week}
\begin{itemize}
	\item keep working on scaling up subst data
