...
\ch done with reciprocal best blast hits (including dealing with multiple best hits)

\ch Queenie: looked into overlapping blocks occurrences
...
I have finally finished my code for getting the reciprocal best blast hits and deal with multiple best hits.
\textbf{Could you let me know if my method seems ok and logical?}
\begin{enumerate}
	\item Find all best hits for each query protein (in AvsB and BvsA blast output). This is based on the lowest e-value score.
	\item If there are multiple best hits (i.e. multiple subject proteins have the same lowest e-value score), calculate the difference in genomic position between the two genes.
	\item The single best hit is determined by the lowest difference in genomic positions (my attempt at synteny)
	\item Find all instances where the best hit for blast A (AvsB) query c is subject d and the best hit for blast B (BvsA) query d is subject c (reciprocal part of the rbbh) 
\end{enumerate}
I was unsure how to best assess synteny because it is supposed to be gene order, but each blast hit only tells you one gene that the query matches to. So I decided to use genomic position (from the gbk files) to tell me if these genes are in the same part of the genome. 
Usually this is accurate and will select the hit that is the same gene (same gene name, protein seq...etc).
However, there are a few cases where a different subject gene is selected because it is closer in genomic position compared to the subject gene with the same gene (same gene name, protein seq..etc).
\textbf{Thoughts on me using the genomic position difference between the query and subject genes as a good measure of synteny?}

Additionally, the proteomes of the \ecol strains are very redundant.
So in the case of \ecol K-12 DH10B, the proteome used was for \ecol K-12 W3110 and for \ecol BW25113 the proteome use was \ecol K-12 MG1655, and finally for \ecol ATCC the proteome used was \ecol CFT073.
This was because the proteomes are redundant and the second proteome had more complete information about gene names. 
\textbf{Does this make sense and seem like a sound thing to do?}

Queenie looked into various aspects of the PARSNP blocks and found that most blocks have an average space between them  of ~700bp. This makes me nervous for when a gene is split between two blocks because if it is split between two blocks that have space between them then we can't really do anything with that gene. So Queenie is checking how often this happens.
It looks like for the overlapping blocks they overlap by a maximum of ~550bp. So if we did ignore these regions I think it is not too bad and we would not lose too much data! I am getting Queenie to quantify how much data this would actually be.

It looks like ATCC is the only strain that has inversions in the overlapping regions (2/8 blocks that are overlapping have an inversion).
Queenie will be looking into these overlapping regions and genes that are split between two blocks to see what kinds of genes they are so we can decided what to do with these regions/genes.
...
%
\begin{itemize}
	\item create map between BW and K-12 genes
	\item get Queenie started on creating lists of which genes match (from all strains) in the parsnp alignment
	\item Queenie to create a toy dataframe that I can begin working on the analysis of comparing inverted and non-inverted sections of the genome
\end{itemize}


\section*{Next Week}
\begin{itemize}
	\item edit another section of dissertation intro
	\item get Queenie to create a plot of the inversions
	\item continue to work on blast BW/K-12 map
\end{itemize}

%\newpage
%
%\begin{figure}
%	\includegraphics[width=0.9\linewidth]{./K12MG_graph.pdf}
%	\caption{\label{fig:exp_cor} }
%\end{figure}
%
%
%
