...
%	
	...
	\item put new 10kb lin reg and \# of genes over 10kb lin reg into paper
	
	\item write about $\uparrow$ in methods and discussion
	
	\item put expression lin reg and \# coding sites log reg into supplement
	
	\item write about $\uparrow$ in paper and how results are the same
	
	\item update supplementary figures/file
	
	\item correlation of gene expression across strains
	
	\item if necessary add a phylogenetic component to the analysis
	
	\item potentially remove genes that have been recently translocated from the analysis
	
	\item model gene exp + position + number of genes
	
	\item split up the strep data into core and non core and see if results are the same
	
	\item what is going on with \strep number of genes changing drastically from core to non-core
	
	\item codon bias?
	
	\item what is going on with really high gene expression bars
...
\ch linear regression on 10kb regions for gene expression (to match number of genes lin reg)

\ch test examples for genes near and far from terminus (robust log reg/results) for number of substitutions

\ch make graphs proportional to length of respective cod/non-cod regions

\ch figure out why the data is weird for number of cod/non-cod sites

Last week I re-did the gene expression linear regression on 10kb regions instead of using each gene as one data point.
This was so that the results are comparable to the number of genes linear regression which was also over 10kb chunks.
I showed you this last week and the results are the same and look good!
We decided to leave the graphs the same (not to divide the expression by the total number of genes in each 10kb section) because they are already the median expression value so it is already showing some sort of average.
These results can be seen in Tables \ref{tab:lr_exp_10kb} and \ref{tab:lr_gene_num} as well as the per gene linear regression results in Table \ref{tab:lr_exp}.

I also took 20kb close and far from the origin for each bacteria in both the protein coding and non-protein coding sections and performed a logistic regression on those sub-sections to see how number of substitutions correlates with distance from the origin of replication.
The results can be found in Table \ref{tab:20kb_near_far_ori}.
Most of the results are negative or not significant, but there are a few that are positive... Just wondering what you thoughts are on this? I will come talk to you about this later.

I told you that there was issues with how my data was being read in and processed which was making the proportionate graphs look really wonky.
Well, I fixed this! 
Turns out my indexing was off and the substitutions data was starting from 0 where as the number of protein coding and non-protein coding sites was starting from 10000, and my code was freaking out when there were no substitutions (because of missing data).
Additionally, these graphs were stacked bar graphs. So I have separated them out and made two graphs, one for protein coding and one for non-protein coding. 
This is all fixed and I have some lovely graphs for you that make perfect sense.
The non-protein coding regions have more substitutions than the protein coding regions, see Figures below.


I also wanted your opinion about something that was mentioned at the conference.
Someone asked me if I was including RNA in my ``coding'' sections? Because RNA often is under different selective pressures than coding or other ``non-coding'' sections, they suggested that I do my analysis on just all RNA to see if there is any sort of trend with respect to distance from the origin.
I was wondering what you think about this and if you think it is worth it for me to do?
...
%
The new weighted graph for \strep is looking a little weird so I need to look into this and make sure that I am not messing something up.

I would like to check if all strains have roughly the same gene expression values per gene to ensure that they are comparable.
I am not sure if I need to do this statistically or if I can just do it ``by eye''. Thoughts?

You might have noticed that the logistic regression for number of substitutions near and far from the origin is missing values for the average substitutions. I still need to calculate this to ensure that the number of substitutions is higher near the origin than at the terminus.
I plan on doing that this week.

I would also like to perform a linear regression on the average (or total?) number of substitutions per 10kb section of the genome.
This would be another check to ensure that the substitution results are robust.

\section*{Next Week}

I would like to really investigate why \strep has such weird values for \dn, \ds, and $\omega$.
I plan on first checking that for all the bacteria $\omega$ is not $>$ 1 for any gene.
Then calculating \dn, \ds, and $\omega$ by hand for a few \strep genes to see what happens.
...
\includegraphics[width=\textwidth]{C:/Users/Daniella/Documents/Sinorhizobium2015/Figs/Sub_graphs_cod_non_cod_7Feb19/ecoli_chrom_cod_non_cod_weighted_sub_histogram_bidirectionality_colour_2Sep19.pdf}

\newpage

\includegraphics[width=\textwidth]{C:/Users/Daniella/Documents/Sinorhizobium2015/Figs/Sub_graphs_cod_non_cod_7Feb19/bass_chrom_cod_non_cod_weighted_sub_histogram_bidirectionality_colour_2Sep19.pdf}

%\includegraphics[width=\textwidth]{C:/Users/Daniella/Documents/Sinorhizobium2015/Figs/Sub_graphs_cod_non_cod_7Feb19/strep_chrom_cod_non_cod_sub_histogram_bidirectionality_colour_9Aug19.pdf}

\includegraphics[width=\textwidth]{C:/Users/Daniella/Documents/Sinorhizobium2015/Figs/Sub_graphs_cod_non_cod_7Feb19/sinoC_chrom_cod_non_cod_weighted_sub_histogram_bidirectionality_colour_2Sep19.pdf}

\includegraphics[width=\textwidth]{C:/Users/Daniella/Documents/Sinorhizobium2015/Figs/Sub_graphs_cod_non_cod_7Feb19/pSymA_chrom_cod_non_cod_weighted_sub_histogram_bidirectionality_colour_2Setp19.pdf}

\includegraphics[width=\textwidth]{C:/Users/Daniella/Documents/Sinorhizobium2015/Figs/Sub_graphs_cod_non_cod_7Feb19/pSymB_chrom_cod_non_cod_weighted_sub_histogram_bidirectionality_colour_2Sep19.pdf}
...
\begin{table}[h]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{lcc|cc|cc|cc}
			...
			\midrule
			\ecol Chromosome &  \cc -3.018\e{-5}* & NS & 2.87\e{-2} & 1.235\e{-2} & \cc -2.884\e{-5}**& \cc -5.276\e{-5}*& 1.085\e{-2} & 4.6\e{-3}\\
			\bass Chromosome & -3.018\e{-5}* & NS & 2.87\e{-2} & 4.235\e{-2} & NS & 5.960\e{-5}**& 4\e{-4} & 6.25\e{-3}\\
			\strep Chromosome & \cc -1.988\e{-5}*** & \cc -5.986\e{-5}*** & 1.265\e{-1} & 2.58\e{-2}& 3.154\e{-5}*** & NS & 2.23\e{-2} & 2.4\e{-3}\\
			\smel Chromosome & 5.109\e{-6}** & NS & 4.1\e{-3} & 2\e{-4}& NS & NS & 9\e{-4} & 1.5\e{-4}\\
			\smel pSymA & NS & NS & 6.15\e{-3} & 1.9\e{-3}& 1.425\e{-4}*** & \cc -1.867\e{-4}*& 2.8\e{-3} & 5.5\e{-4}\\
			\smel pSymB & NS & \cc -4.411\e{-5}*** & 3.37\e{-2}& 2.845\e{-2}& NS & \cc -4.669\e{-5}**& 7\e{-3} & 6.05\e{-3}\\
			\bottomrule
		\end{tabular}
...
\begin{table}[h]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{lcc|cc}
			...
			\midrule
			\ecol Chromosome &   & & &\\
			\bass Chromosome & NS & \cc -1.761\e{-4}** & NS & \cc -2.47\e{-5}** \\
			\strep Chromosome & NS &\cc -4.382\e{-4}***& 1.87\e{-9}*& \cc -6.08\e{-5}***\\
			\smel Chromosome &\cc -1.836\e{-10}* &\cc -1.467\e{-5}**& NS & NS\\
			\smel pSymA & NS & NS &\cc -1.266\e{-8}**& \cc -6.74\e{-5}**\\
...
\begin{table}[h]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{lc}
			...
			\midrule
			\ecol Chromosome & NS\\
			\cc \bass Chromosome & \cc -2.682\e{-6}*** \\
			\strep Chromosome & -2.360\e{-6}***\\
			\smel Chromosome & -2.074\e{-6}***\\
			\smel pSymA & NS \\
			\smel pSymB & -4.19\e{-6}*\\
			\bottomrule
		\end{tabular}
...
\begin{table}[h]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{lcc}
			...
			\midrule
			\ecol Chromosome & \cc -1.354\e{-7}*** & NS \\
			\bass Chromosome & \cc -6.735\e{-8}*** & NS \\
			\strep Chromosome & 4.105\e{-7}*** & 1.635\e{-7}***\\
			\smel Chromosome & \cc -9.185\e{-8}***  & \cc -1.749\e{-7}*** \\
			\smel pSymA & \cc -8.121\e{-7}*** & \cc -1.247\e{-6}*** \\
			\smel pSymB & 1.655\e{-7}*** & 4.105\e{-7}***\\
			\bottomrule
		\end{tabular}
