...
\ch linear regression on 10kb regions for substitutions (weighted and non-weighted) (Table \ref{tab:lin_reg_10kb_subs})

\ch average number of substitutions in 20kb regions near and far from the origin (Table \ref{tab:20kb_near_far_ori})

\ch add code to check protein cod subs $>$ non-protein coding subs

\ch figured out weird \strep number of cod and non-cod sites (Figure below)

\ch correlation of gene expression across strains (Figures  \ref{fig:ecoli_gene_exp_mean} and \ref{fig:bass_gene_exp_mean})
...
I realized that for \strep, the weird number of protein coding and non-protein coding sites was because of zero substitutions in some sections messing up the proportional calculation.
The new graph is below!

As I mentioned to you I looked into the correlation of gene expression across strains for that paper and it looks like all the datastes are roughly the same, which is great!
\strep, and all the replicons of \smel only had one dataset, so I could not look into this with them.
I thought that \strep had more than one dataset but I was wrong and accidentally counted one of the replicates as it's own dataset!
The weird genes that had super high expression were all hypothetical proteins or unknown proteins.
The graphs for \ecol and \bass are in Figures \ref{fig:ecoli_gene_exp_mean} and \ref{fig:bass_gene_exp_mean}.
...
As mentioned above I need to re-look at why the proportionate number of substitutions is higher in the protein coding regions than non-protein coding sections.
I think this has to do with how I split up the alignment into cod and non-cod sections so I will be looking into this further this week.
...
\newpage
\begin{figure}
	\includegraphics[width=\textwidth]{C:/Users/Daniella/Documents/Sinorhizobium2015/Gene_Expression_8Dec17/Gene_Expression/Ecoli_exp_normalization_combining_8Mar18/ecoli_median_mean_gene_exp_check.pdf}
	\caption{\label{fig:ecoli_gene_exp_mean} Distribution of the median expression value for each \ecol dataset minus the mean expression value for that gene across all datasets. Each gene is shown on the x-axis and the log base 10 values are on the y-axis.}
\end{figure}

\begin{figure}
	\includegraphics[width=\textwidth]{C:/Users/Daniella/Documents/Sinorhizobium2015/Gene_Expression_8Dec17/Gene_Expression/Bass_exp_normalization_combining_6Mar18/bass_median_mean_gene_exp_check.pdf}
	\caption{\label{fig:bass_gene_exp_mean} Distribution of the median expression value for each \bass dataset minus the mean expression value for that gene across all datasets. Each gene is shown on the x-axis and the log base 10 values are on the y-axis.}
\end{figure}	
...
\includegraphics[width=\textwidth]{C:/Users/Daniella/Documents/Sinorhizobium2015/Figs/Sub_graphs_cod_non_cod_7Feb19/strep_chrom_cod_non_cod_weighted_sub_histogram_bidirectionality_colour_6Sep19.pdf}
