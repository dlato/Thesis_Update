...
\ch gene expression edits/wrote more

\ch take into account number of genes (normalized gene expression based on this)

\ch figure out what is going on with \strep number of genes drastically increasing after core region

\ch investigate really high gene expression bars in all bacteria

\ch start new graph with negative positions (subs and gene expression)

\textbf{Gene Expression Paper}
I did a lot of work over the weekend editing my gene expression paper and adding in more information. 
I have another few sections ready for you to edit, so let me know if you want me to send those to you.

\textbf{\strep uneven number of gene}
Trying to figure out why there seemed to be such a jump in the total number of genes for my gene expression data at around 2260000 from the origin (bidirectionally).
Looking into this I realised that it is because this is where  the end of the short arm of the chromosome ends.
So, because I am calculating my positions bidirectionally, they are ``folding'' the two chromosome arms together, artificially increasing the total number of genes in that section of the graph.
I separated this out into each of the replicon halves, and still got the same answer, where the total number of substitutions were larger near the origin of replication and smaller near the terminus.
This also sparked discussion about how I should be visualising this.
There are some papers that use negative numbers to indicate the left half of the replichore and positive numbers to indicate the right half of the replichore, leaving the origin of replication somewhere in the middle of the graph.
I am fine with presenting the data this way, but then I think that I should be doing my regression analysis on both replichore halves separately, because I can't really treat it all as one if I am showing the graph with negative and positive numbers. 
Thoughts?
%As discussed, the short segments of the alignments are often inaccurate and not comparing homologous regions. So we decided to make each segment a minimum of 100bp long.
%This means that my code will go through the mafft alignment first, making sure that segments are comparing codon 1 with codon 1...etc. 
%And then those sections will be passed through Gblocks to ensure that they are min 100bp and well aligned.

\textbf{High Gene expression bars in all bacteria?}
I looked into the high gene expression bars in all the bacteria and found that they are mostly full of ribosomal proteins, DNA replication and repair proteins, and translation modification (see below Table \ref{tab:high_exp_bars}).
This is consistent across all the bacteria! Which makes sense why these bars are so much higher than the others!
It is super interesting that these are not always found near the origin of replication!
I added this explanation into my paper draft.


\textbf{Normalising by number of genes}
I have been thinking about the number of genes in each 10kb section and how this may potentially be impacting/biasing my gene expression results.
So I decided to do a better normalisation of the gene expression and the number of genes.
I took the total expression value per 10kb chunk of the genome (added up all gene expression values for each 10kb region), then divided this by the total number of protein coding genes in that section.
I then did a linear regression on these values, and still obtained the same result (Table coming soon).
Not sure which of these tables should be the ``main result'' found in the paper and which should be supplemental? thoughts?
I was also thinking that I should probably have one graph with these normalised by gene number values, to avoid having two graphs ontop of eachother (one for gene expression and one for the total number of genes). 
Thoughts?
...
\begin{enumerate}
	\item  pick which group of \strep genomes to use
	\item what to do about length cut off for the non-coding regions? am I putting this whole section in the supplement?
	\item assess the \borrb mauve plots for similar seqs
	\item align the \agro taxa into more specific phylo groups with mauve
	\item assess the agro mauve plots
	\item run \ecol through the new pipeline to work out any kinks
\end{enumerate}


\section*{Next Week}
Substitutions project continued:
\begin{enumerate}
	\item run either \strep or one of the other linear bacteria through the substitutions pipeline
	\item run the rest of the bacteria through the new pipeline
	\item new graphs for gene expression? (negative positions)
...
\begin{table}[h]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{lcc}
			...
			\midrule
			\ecol Chromosome & \cc -4.308\e{-8}*** & NS \\
...
\begin{table}[h]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{lcc|cc|cc|cc}
			...
			\bass Chromosome & NS & 1.863\e{-5}* & 4.8\e{-3} & 3.06\e{-2} & 1.017\e{-4}* & 5.823\e{-5}***& 8\e{-4} & 6.75\e{-3}\\
			\strep Chromosome && &  & &  &  &  & \\
			...
			\smel pSymB & \cc -1.553\e{-5}* & \cc -4.908\e{-5}*** & 3.23\e{-2} & 2.36\e{-2} & NS & \cc -4.557\e{-5}**& 5.1\e{-3} & 5.4\e{-3}\\
			\bottomrule
		\end{tabular}
