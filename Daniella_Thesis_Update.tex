...
\ch did HGT correlations with Oshima data, still no correlation

\ch looked into other HGT and HNS binding data

\textbf{Dissertation:}

\ch first draft of Exit Seminar presentation

\ch practising exit seminar

\ch First draft of dissertation done!!
...
I looked at finding other HGT data and HNS binding site data, but there really is not too much else.
The HGT data is from early 2000s but is still being used frequently today (including in the most recent HNS study I could find).
For the HNS binding sites, I found a few other data sets that I could be using, but they split up the binding sites into different growth phases (aerobic anerobic... etc.).
\textbf{None of my other HNS data are partitioned like that. Should I be including these new datasets?}
...
\item DESeq analysis for inversions

\item graphs for $\uparrow$

\item write methods and results for DESeq analysis

\item read some papers

\item send Brian first draft of thesis!
...
\section*{Next Week}
\begin{itemize}
	\item write up DESeq discussion and conclusion
	\item exit seminar
