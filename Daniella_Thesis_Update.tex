...
\textbf{Subst Paper:}
The results from the ``leave one out'' analysis can be found in Tables \ref{tab:leave_one_out1} and \ref{tab:leave_one_out2}.
It looks like the results are mostly consistent, so when you leave a taxa out, the sign remains unchanged. There are a few exceptions which I have highlighted in red. I will be looking into these further.
I am wondering if this could be because I used the maximum genome position when scaling my points to the origin of replication as the longest genome out of all the taxa, instead of the taxa that are used in each ``leave one out''. I will explore this to see if I missed something or messed up along the way.
\textbf{Thoughts on these results so far? }
...
\item check into block lengths and inversions results
\item choose log reg values for final table
\item actual analysis on DESeq data
\item visualizations/results for $\uparrow$
\item continue working on other edits for subst paper
...
\begin{itemize}
	...
	\item check how many HNS binding sites there are for each dataset
	\item think about how to visualize H-NS and inversions info
	\item get Lang and Oshima data from PDF to csv formats
	\item do H-NS analysis on $\uparrow$
	\item final decision on inversion viz
	\item continue working on other edits for subst paper
\end{itemize}
