...
\ch Queenie: summary graphs and stats about the inversions

\ch Queenie created dataframe with raw expression data and inversions info

\ch Queenie created dataframes with various taxa present

\ch Queenie: started to compare outputs from various blast parameters

\ch preliminary analysis on dataframes with various taxa

\ch started implementing origin/bidirectional scaling of the inversion/gene expression values

\ch Attempt to create \p like picture of inversions

Subst Paper:

\ch chose 26 random \ecol, 25 random \bass, and 26 random \strep genomes to begin looking at
...
Queenie finished creating graphs summarizing the gene expression, length and position of inverted and non-inverted genomes.
However, they are not really informative and I think might just be better as tables showing the averages.
It appears as though only some blocks have significant differences in those categories so to try and show all blocks at once does not visually create any stark difference between inversions and non-inversions.

Queenie also finished creating dataframes with multiple combinations of taxa present in each block so we could see if it made a difference.
I have completed preliminary analysis on these dataframes and the numbers are very similar, so there might not be any huge difference in including blocks with less than all taxa (Table \ref{tab:multi_taxa_inver}).

Queenie has started to write code to compare the reciprocal best blast hits from the various parameters I chose.
There were some interesting issues she brought up to me such as not all blast gene names are present in the genbank file.
I am not sure what is going on here but the gene names just do not exist in the genbank file for some genes.
\textbf{any thoughts on this? Should I be overly concerned?}

I started to implement the same origin/bidirectional scaling code used in my other papers and noticed that the strand information was missing from the dataset. 
I am getting Queenie to look into this.

I tried to make a \p-like picture of the genomic inversions but had trouble.
The PARSNP output can not be read into \p (still looking into this but it does not look possible) so I can not use \p to create the diagram.
The visualization produced by PARSNP is not as useful because it does not clearly connect blocks that have been shuffled around the genome.
I tried to look into a program called GenomeDiagram which was used in one of the Galardini papers on \s. 
However, this required an updated version of BioPython on the machines and only reads in genbank files (from my knowladge) so I am not sure how to convert the PARSNP outputs to create the ``features'' needed to create the same diagram.
The other option (which would be easiest) is to just use R to plot the x-y coordinates from matching genes (either from the blast output or my gene mapping code).
...
I think that the reason that the analysis looks ``fine'' for the 23 \smel genomes is that they are quite similar, and therefore aligning them does not pose any issues.
So I decided to test out how long the alignment would take and how much would be trimmed with $~$25 random (complete) genomes from \ecol, \bass, and \strep.
I have updated the \p time plots with the new values for \bass and \ecol (Figures \ref{mauve_60} and \ref{mauve_100}).
The results are still roughly the same, that having about 60 sequences would take over a month to align.
The alignments do appear to be ``messy'' with 96 block for \ecol (Figure \ref{ecoli26_mauve}) and 113 blocks for \bass (Figure \ref{bass25_mauve}).

I do not have a plot of how long the individual block re-alignments with \texttt{MAFFT} take (\textbf{I can make one if you think it is useful?}) but it takes about 5h for the longest \ecol block to align.
...
%
\begin{itemize}
	\item Queenie: compare blast results and gene mapping
	\item Queenie: combine gene mapping and gene info (from gbk)
\item continue scaling up the \ecoli, \bass, and \strep genomes 
...
\item find new outgroups for subst trees
\item assess Queenie's results from various blast parameters
...
\begin{itemize}
	...
	\item Queenie: determine which RBBH's are the same as the homologous genes identified from the alignments
	\item keep working on increased number of genomes for subst analysis
	\item distinction between mutations and substitutions in subst paper intro
\end{itemize}

\newpage
\begin{figure}
	\includegraphics[width=\textwidth]{C:/Users/synch/Documents/PhD/Substitutions_paper/mauve_time_plot_extended_60.pdf}
	\caption{\label{mauve_60}}	
\end{figure}

\begin{figure}
	\includegraphics[width=\textwidth]{C:/Users/synch/Documents/PhD/Substitutions_paper/mauve_time_plot_extended_100.pdf}
	\caption{\label{mauve_100}}	
\end{figure}

\begin{figure}
	\includegraphics[width=\textwidth]{26_Ecoli_chromosomes_mauve_pic_3Aug20.jpg}
...
\begin{table}[t!]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{lccc}
			...
			\cmidrule{2-4}
			Datasets:  & Inverted & Inverted and &Increased in Gene Expression\\
			Taxa Per Block &  & Differentially Expressed&in the Inverted Sequences \\
			\midrule
...
%			\midrule
%			\textbf{\smel Chrom + \agro} & 12.5529 & 0.0553 & 0.0265 \\
...
%			\smel pSymA &  0.0835 & 0.0099 & 0.1645 \\
...
%	\begin{subfigure}{.5\textwidth}
%		\centering
%		\includegraphics[width=0.9\linewidth]{C:/Users/synch/Documents/PhD/Substitutions_paper/GBE_paper_format/figs/ecoli_dN_dS_omega_dist_31Mar20.pdf}
%		\caption{}
%		\label{fig:e2}
%	\end{subfigure}
%	\label{fig:ecoli}
...
%	\begin{subfigure}{.5\textwidth}
%		\centering
%		\includegraphics[width=0.9\linewidth]{C:/Users/synch/Documents/PhD/Substitutions_paper/GBE_paper_format/figs/bass_dN_dS_omega_dist_31Mar20.pdf}
%		\caption{}
%		\label{fig:b2}
%	\end{subfigure}
%	\label{fig:bass}
...
%	\begin{subfigure}{.5\textwidth}
%		\centering
%		\includegraphics[width=0.9\linewidth]{C:/Users/synch/Documents/PhD/Substitutions_paper/GBE_paper_format/figs/strep_dN_dS_omega_dist_31Mar20.pdf}
%		\caption{}
%		\label{fig:s2}
%	\end{subfigure}
%	\label{fig:strep}
...
%	\begin{subfigure}{.5\textwidth}
%		\centering
%		\includegraphics[width=0.9\linewidth]{C:/Users/synch/Documents/PhD/Substitutions_paper/GBE_paper_format/figs/sinoC_dN_dS_omega_dist_31Mar20.pdf}
%		\caption{}
%		\label{fig:sC2}
%	\end{subfigure}
%	\label{fig:sinoC}
...
%	\begin{subfigure}{.5\textwidth}
%		\centering
%		\includegraphics[width=0.9\linewidth]{C:/Users/synch/Documents/PhD/Substitutions_paper/GBE_paper_format/figs/pSymA_dN_dS_omega_dist_31Mar20.pdf}
%		\caption{}
%		\label{fig:sP2}
%	\end{subfigure}
%	\label{fig:pSymA}
...
%	\begin{subfigure}{.5\textwidth}
%		\centering
%		\includegraphics[width=0.9\linewidth]{C:/Users/synch/Documents/PhD/Substitutions_paper/GBE_paper_format/figs/pSymB_dN_dS_omega_dist_31Mar20.pdf}
%		\caption{}
%		\label{fig:sB2}
%	\end{subfigure}
%	\label{fig:pSymB}
